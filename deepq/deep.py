...
class ReplayMemory:
    ...
    def get_minibatch(self):
        if self.count < self.agent_history_length:
            raise ValueError('Not enough memory to sample minibatch')
...
def play_one(
    ...
    return total_t, episode_reward, (
            datetime.datetime.now() - t0), num_steps_in_episode, total_time_training / num_steps_in_episode, epsilon
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            sys.stdout.flush()
        print("Total duration:", datetime.datetime.now() - t0)
